	...
	<div class="containeur_profil">
		...
		<div class="container_users-table">
			<table id="users-table">
				...
				<?php
        $result = $conn->query("SELECT username, age, department, ville_users, gender, profile_image FROM users");
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>";
            if ($row['profile_image']) {
                $imageData = base64_encode($row['profile_image']);
                echo "<img src='data:image/jpeg;base64,{$imageData}' alt='Profile Image' width='50' height='50'>";
            } else {
                echo "Pas d'image";
            }
            echo "</td>";
